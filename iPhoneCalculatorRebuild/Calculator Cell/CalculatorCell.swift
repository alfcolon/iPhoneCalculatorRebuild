...
enum CalculatorCell: AttributedString, CellAppearence, ToggledValue {
	...
	var accessibilityLabel: String {
		...
		case .equal:
			return "equal"
		case .exponentFunction(let type):
			switch type {
			...
			case .BaseXPowerY: // = "xy"
				return "power"
			case .BaseYPowerX: // = "yx"
				return "reverse power"
			case .EnterExponent: // = "EE"
				return "E.E. engineering exponent"
			}
		case .factorial(let type):
			switch type {
			case .Factorial:
				return "factorial"
			}
		case .logFunction(let type):
			switch type {
			case .LogBaseTen:
				return "log base ten"
			case .LogBaseTwo:
		...
		case .toggleSecondSetOfFunctions:
			return "alternative trigonometry functions"
		case .trigonometricHyperbolicFunction(let type):
			switch type {
			case .Cosine:
		...
		case .trigonometricInverseHyperbolicFunction(let type):
			switch type {
			case .Cosine:
		...
		case .trigonometricInverseRightAngleFunction(let type):
			switch type {
			case .Cosine:
		...
		case .trigonometricRightAngleFunction(let type):
			switch type {
			case .Cosine:
				return "cosine"
			case .Sine:
	...
	var togglesSecondSecondSetOfFunctions: Bool {
		switch self {
		case .toggleSecondSetOfFunctions:
			return true
		default:
	...
	var toggleClearType: Bool {
		...
		// This should actually toggle AC to C
//		return  self.isFunctionWithTwoInputs
	}
	...
	var deselectsSelectedCell: Bool {
		switch self {
		case.constant,
			.decimal,
			.digit,
